...
int main(){
    ...
    // Began my own code here. Rest of the code was provided by my professor, however, I went through everything and understand it.
    BmodeClass *currentScanline = scanlineHead; // declare bmodeclass pointer called currentScanline and assign scanlineHead to it
    dataBuffer *curData = dataHead;
    for (int i = 1; i < parameters -> getNumScanline(); i++){ //iteratively build up a BmodeClass linked list
        curData = curData -> next;
        currentScanline->next = new BmodeClass(parameters, curData, i); //Create a new BmodeClass object
        currentScanline = currentScanline->next; //assign the newly created object to the pointer next of currentScanline
        currentScanline -> next = NULL; //advance the pointer currentScanline to the next entry
        
    }
   // stopped my own code here.
